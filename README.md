...
枚举所有版本组合**并发探测**,输出全部可下载的完整 URL。
支持从单一平台的 URL 发现其他平台的下载地址(高级模式)。
...
```bash
# 识别 1.17.4, 枚举 0.0.0 ~ 1.17.7(含已知版本之后 3 个)全部组合
...
| `-universe` | `199` | 版本分量最大值 0..universe(=200 个取值, 即 200×200×200 上限) |
| `-stop` | `6` | 滚动窗口模式: 连续未发现可下载版本多少次即停止(命中立即刷新窗口); 候选空间过大时自动切换此模式 |
| `-front-stop` | `5` | 前沿探测中连续未命中多少次即停止该维度 |
| `-from` | `0.0.0` | 起始版本(含) |
| `-to` | 识别版本 + beyond | 结束版本(含) |
| `-beyond` | `3` | 历史区之上再探测的更新版本数量 |
| `-c` | `50` | 并发数 |
| `-timeout` | `10s` | 单请求超时 |
| `-retry` | `1` | 网络错误/429/5xx 的额外重试次数 |
| `-max` | `200000` | 总探测请求上限(含历史区与前沿区) |
...
| Qianwen V4.1.0.175 dmg | — | — | 正确判为不可遍历(版本+Build混合) |
...
https://pc-download.qianwen.com/download/37271/qianwenmac/pcqwen@default/QianwenMac_V4.1.0.175_mac_pf3000_(zh-cn)_releasemini_(Build3130705).dmg?response-content-disposition=attachment;%20filename=%22Qianwen_V4.1.0.175_fp_dapi-0eff522d-6e38-4329-86ce-2d21e7ec71b9_fp.dmg%22;filename*=UTF-8%27%27Qianwen_V4.1.0.175_fp_dapi-0eff522d-6e38-4329-86ce-2d21e7ec71b9_fp.dmg
